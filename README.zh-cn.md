...
AI Agent 技能生态是新的软件供应链 — 正在遭受攻击。技能以 **Agent 完整权限** 运行，**默认无沙箱**，发布门槛仅需一个 Markdown 文件。IpMan 提供：

- **安装前风险评估** — 每个技能自动扫描凭证窃取、数据外泄、混淆代码、prompt 注入等红旗
- **四级安全模式** — 从 PERMISSIVE（全部安装）到 STRICT（仅安装安全技能）
- **社区驱动的威胁举报** — 标记可疑技能，举报次数纳入风险评分
- **发布时门禁** — HIGH/EXTREME 风险技能在发布环节即被阻止
