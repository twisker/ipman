...
### 2.1 产品范围
IpMan 是一个纯命令行工具，专注于 Agent 技能的环境管理。它允许用户创建、切换、删除虚拟环境，安装、卸载、发布技能和 IP 包，并与在线市场交互。IpMan 不修改 Agent 工具的内部实现，仅通过调用 Agent 的标准技能管理接口或操作技能存放目录（通过软链接等方式）来实现隔离。

### 2.2 用户特征
- **技能开发者**：需要为不同项目隔离技能环境，避免版本冲突；希望发布自己的技能或 IP 包到市场。
...
- 在线市场利用 GitHub 等免费基础设施实现数据存储（如使用 GitHub Issues、GitHub Pages 或 GitHub Actions 进行简单计数和元数据存储）；若不可行，则使用低成本 OSS 方案。
...
| FR9 | 支持从在线市场按简单名称安装技能或 IP 包（参考 pip 的 `install <package>`）。 | 细节要求8 |
...
| 需求ID | 描述 | 来源 |
|--------|------|------|
| FR16 | 提供一个在线 IP 市场，支持用户搜索、浏览、下载技能和 IP 包。 | 主要目标5 |
| FR17 | 市场应记录每个技能和 IP 包的安装/下载次数，并支持排名（Top 10/20/50）。 | 细节要求6，20 |
| FR18 | 支持将本地的技能或 IP 包发布到市场（需用户认证，认证方式待定）。 | 细节要求8 |
| FR19 | 市场的持久化数据应优先利用 GitHub 免费资源实现（如 Issues、Pages、Actions 等）；若不可行，则使用公共免费基础设施；最后考虑低成本 OSS。 | 细节要求9 |
...
| 需求ID | 描述 | 来源 |
|--------|------|------|
| NFR15 | 在线市场的安装/下载量统计数据应准确记录，并用于排名展示。 | 细节要求6，20 |
...
### 5.3 与在线市场的接口
- **HTTP/HTTPS 通信**：用于从市场下载技能元数据和包内容、上传发布包、获取统计数据等。市场可能提供 RESTful API。
